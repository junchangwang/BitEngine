...
void BMTableScan::TPCH_Q8_Orders_GetRowIds(ExecutionContext &context, vector<row_t> *row_ids)
{
	...
	SegBtv *btv_orderdate = rabit_orderdate->range_res(0, left_days - 1 , 731 );
	...
	std::cout << "orderdate's bitmap time : " << std::chrono::duration_cast<std::chrono::milliseconds>(s1 - s0).count() << "ms" << std::endl;
...
SourceResultType BMTableScan::BMTPCH_Q8(ExecutionContext &context, DataChunk &chunk, const TableScanBindData &bind_data)
{
	if(*cursor == 0) {
			TPCH_Q8_Orders_GetRowIds(context, row_ids);
		...
		if(*cursor < row_ids->size()) {
			...
			storage_column_ids.push_back(StorageIndex(4));	// For o_orderdate
