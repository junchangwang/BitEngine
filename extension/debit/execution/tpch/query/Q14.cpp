...
void BMTableScan::TPCH_Q14_Lineitem_GetRowIds(ExecutionContext &context, vector<row_t> *row_ids)
{
	...
	SegBtv* btv_internal = rabit_shipdate->Btvs_GE[n_month]->seg_btv;
	SegBtv* btv_shipdate = new SegBtv(*btv_internal);
	btv_shipdate->deepCopy(*btv_internal);
	btv_shipdate->decompress();

	// SegBtv *btv_shipdate = rabit_shipdate->range_res(0, lower_days, upper_days - lower_days);
...
SourceResultType BMTableScan::BMTPCH_Q14(ExecutionContext &context, DataChunk &chunk, const TableScanBindData &bind_data)
{
    if(*cursor == 0) {
